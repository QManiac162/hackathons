...
function createParticleSystem(name) {
    const capacity = 10000;
    let trace;
    if (GPUParticleSystem.IsSupported)
        trace = new GPUParticleSystem(name, { capacity }, global.scene);
    else
        trace = new ParticleSystem(name, capacity, global.scene);
    ...
    trace.emitter = global.scene.meshes.find((m) => m.name === name);
    trace.start();
    return trace;
}
export function createSolarBody(name, position) {
    const desc = bodies.find((b) => b.englishName === name) || {};
    log('create', { name, position, desc });
    const diameterX = (desc.equaRadius ** global.sizeExponent) * global.sizeFactor;
    const diameterY = desc.polarRadius !== 0 ? (desc.polarRadius ** global.sizeExponent) * global.sizeFactor : diameterX;
    const diameterZ = (desc.equaRadius ** global.sizeExponent) * global.sizeFactor;
    const sphere = MeshBuilder.CreateSphere(name, { diameterX, diameterY, diameterZ, segments: 20 }, global.scene);
    const material = new StandardMaterial(name, global.scene);
    ...
    sphere.rotation = new Vector3(0, Math.PI / 2, Math.PI / 2 - ((desc.axialTilt || 0) * Math.PI / 360));
