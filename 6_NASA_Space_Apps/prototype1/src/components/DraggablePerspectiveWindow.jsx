...
const DraggablePerspectiveWindow = () => {
  ...
  starSystemsData.starSystems.forEach(system => {
    ...
    system.planets.forEach(planet => {
      ...
      planet.moons?.forEach(moon => allBodies.push(moon.name));
    });
  });
      ...
      for (const planet of system.planets) {
        ...
        if (planet.moons) {
          for (const moon of planet.moons) {
            if (moon.name === bodyName) {
              return moon.color; // Assuming 'color' is a property of the moon
            }
          }
        }
      }
    ...
    for (const system of starSystemsData.starSystems) {
      ...
      for (const planet of system.planets) {
        ...
        if (planet.moons) {
          for (const moon of planet.moons) {
            if (moon.name === bodyName) {
              return { x: planet.distance + moon.distance, y: 0, z: 0 }; // Placeholder position
            }
          }
        }
      }
