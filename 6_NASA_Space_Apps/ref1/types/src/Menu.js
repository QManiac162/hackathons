...
export async function createMenu() {
    ...
    if (!global.menu)
        global.menu = new Menu(document.body, 'solar system simulation', { top: '20px', right: '20px' });
    ...
    global.menu.addButton('show inspector', 'hide inspector', () => {
        if (!global.scene)
            return;
        const globalRoot = document.getElementById('inspector');
    ...
    global.menu.addList('simulation centric view', ['sun', 'earth'], 'sun', (item) => {
        ...
        if (!global.scene)
            return;
        if (item === 'sun') {
            global.scene.activeCamera.alpha = Math.PI / 2;
            global.scene.activeCamera.beta = Math.PI / 2;
        }
        if (item === 'earth') {
            global.scene.activeCamera.alpha = 0;
            global.scene.activeCamera.beta = -3 * Math.PI / 4;
        }
    });
