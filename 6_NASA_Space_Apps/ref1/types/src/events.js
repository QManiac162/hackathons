...
export function pointerDown() {
    ...
    if (pickInfo?.pickedMesh) {
        ...
        global.scene.activeCamera.setTarget(pickInfo?.pickedMesh);
    }
    else {
...
export function pointerMove() {
    document.getElementById('desc').style.display = global.tooltip ? 'block' : 'none';
    if (!global.scene)
        return;
    const pickInfo = global.scene.pick(global.scene.pointerX, global.scene.pointerY, undefined, false, global.scene.activeCamera);
    ...
    for (const [key, val] of Object.entries(desc)) {
        if (key === 'id' || key === 'isPlanet' || key === 'name')
            continue;
        if (val === '' || `${val}`.startsWith('http') || val === null)
            continue;
        let v = val;
        if (Array.isArray(val))
            v = val.length;
        if (typeof val === 'object')
            v = JSON.stringify(val, null, 2).replace(/{|}|"|,/g, '').replace('  ', ' ');
        str += `${key} ${v}<br>`;
    }
