...
export function createScene() {
    ...
    if (!global.engine) {
        const options = { preserveDrawingBuffer: true, stencil: true, disableWebGL2Support: false, doNotHandleContextLost: true, audioEngine: false };
    ...
    if (global.scene) {
        for (const node of global.scene.getNodes())
            node.dispose();
    ...
    global.engine.runRenderLoop(() => global.scene.render());
    // glow layer
